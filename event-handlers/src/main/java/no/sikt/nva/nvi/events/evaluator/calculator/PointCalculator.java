...
public class PointCalculator {
  ...
  private final Collection<NviCreator> verifiedNviCreators;
  private final Collection<NviCreator> unverifiedNviCreators;
  ...
  private static Map<URI, Long> getCreatorCountPerInstitution(Collection<NviCreator> nviCreators) {
    return nviCreators.stream()
        .flatMap(creator -> creator.topLevelNviOrganizations().stream())
        .map(Organization::id)
        .distinct()
        .collect(
            Collectors.toMap(
  ...
  private static Long countCreators(URI institutionId, Collection<NviCreator> nviCreators) {
    return nviCreators.stream().filter(isAffiliatedWithTopLevelOrganization(institutionId)).count();
  }

  private static Stream<CreatorAffiliationPoints> calculatePointsForAffiliation(
      Entry<URI, Set<URI>> nviCreator, BigDecimal institutionPoints, Long institutionCreatorCount) {
  ...
  private List<CreatorAffiliationPoints> calculateAffiliationPoints(
    ...
    return verifiedNviCreators.stream()
        .filter(isAffiliatedWithTopLevelOrganization(institutionId))
        .collect(
            Collectors.toMap(
                NviCreator::id, creator -> creator.getNviAffiliationsPartOf(institutionId)))
        .entrySet()
        .stream()
        .flatMap(
            entry ->
