...
public class RedriveUpsertDlqHandler implements RequestHandler<RedriveUpsertDlqInput, Void> {
    ...
    public static final String COULD_NOT_EXTRACT_PUBLICATION_ID_FROM_MESSAGE = "Could not extract publicationId from "
                                                                               + "message: ";
    public static final String DUPLICATE_MESSAGE_FOUND = "Duplicate message found for URI: {}, keeping first "
                                                         + "occurrence";
    ...
    private static final String HANDLER_FINISH_REPORT_LOG = "Redrive UpsertCandidateDLQ finished. In total {} "
                                                            + "messages processed";
    private static final String REDRIVE_DLQ_STARTED_LOG = "Redrive UpsertCandidateDLQ started. {} messages to process.";
    private static final String REPUBLISHING_TO_EVALUATION_QUEUE_LOG = "Republishing to ResourceEvaluationQueue: {}";
    ...
    public Void handleRequest(RedriveUpsertDlqInput input, Context context) {
        ...
        while (messageCount > 0) {
            ...
            processedMessages.forEach(receiptHandle -> queueClient.deleteMessage(dlqQueueUrl, receiptHandle));
    ...
    private Set<String> processMessages(NviReceiveMessageResponse response) {
        ...
        var uniqueMessages = response.messages()
                                 .stream()
                                 .collect(Collectors.toMap(this::extractS3UriFromMessage, Function.identity(),
                                                           this::handleDuplicate));

        return uniqueMessages.entrySet()
                   .stream()
                   .map(entry -> processEntry(entry.getKey(), entry.getValue().receiptHandle()))
                   .collect(Collectors.toSet());
    }

    private NviReceiveMessage handleDuplicate(NviReceiveMessage existing, NviReceiveMessage duplicate) {
    ...
    private String processEntry(URI uri, String receiptHandle) {
        ...
        var messageToRepublish = attempt(
            () -> dtoObjectMapper.writeValueAsString(persistedResourceMessage)).orElseThrow();
    ...
    private URI extractS3UriFromMessage(NviReceiveMessage message) {
        return attempt(() -> dtoObjectMapper.readValue(message.body(), CandidateEvaluatedMessage.class)).map(
                CandidateEvaluatedMessage::candidate)
                   .map(this::extractS3UriFromMessage)
                   .orElseThrow(
                       new IllegalStateException(COULD_NOT_EXTRACT_PUBLICATION_ID_FROM_MESSAGE + message.body()));
    }
