 ...
 * created through the regular evaluation flow (see {@link EvaluationContext}). The index document
 * is generated from the persisted Candidate plus the expanded publication read from S3 (via {@link
 * PublicationLoaderService}); there is no live organization lookup, so the generated document
 * reflects production behaviour without any Cristin stubbing.
 */
