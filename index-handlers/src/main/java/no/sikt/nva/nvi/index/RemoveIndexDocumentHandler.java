...
public class RemoveIndexDocumentHandler implements RequestHandler<SQSEvent, Void> {
  ...
  public RemoveIndexDocumentHandler() {
    this(MultiIndexWriter.fromEnvironment());
  }

  public RemoveIndexDocumentHandler(MultiIndexWriter multiIndexWriter) {
  ...
  private void removeDocumentFromIndex(UUID identifier) {
    attempt(() -> multiIndexWriter.removeDocumentFromIndex(identifier))
        .orElse(
            failure -> {
