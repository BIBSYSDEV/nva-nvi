...
public record NviContributor(
  ...
  public List<URI> getOrganizationsPartOf(URI topLevelOrg) {
    ...
    organizationsPartOfTopLevelOrg.addAll(
        getOrganizationsAboveAffiliationInOrgHierarchy(topLevelOrg));
  ...
  public Stream<NviOrganization> getAffiliationsPartOfOrEqualTo(URI topLevelOrg) {
    return nviAffiliations().stream()
        .filter(organization -> isPartOfOrEqualTo(topLevelOrg, organization));
  }
  ...
  private List<URI> getAffiliationsIdsPartOf(URI topLevelOrg) {
    return getAffiliationsPartOfOrEqualTo(topLevelOrg).map(NviOrganization::id).toList();
  }
