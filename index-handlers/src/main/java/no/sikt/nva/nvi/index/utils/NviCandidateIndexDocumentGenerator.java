...
public final class NviCandidateIndexDocumentGenerator {
    ...
    public static NviCandidateIndexDocument generateDocument(String resource, Candidate candidate) {
    ...
    private static NviCandidateIndexDocument createNviCandidateIndexDocument(JsonNode resource, Candidate candidate) {
    ...
    private static List<Contributor> extractContributors(JsonNode resource) {
        return getJsonNodeStream(resource, JSON_PTR_CONTRIBUTOR).map(
            NviCandidateIndexDocumentGenerator::createContributor).toList();
    }
    ...
    private static List<String> extractAffiliations(JsonNode contributor) {
        return streamNode(contributor.at(JSON_PTR_AFFILIATIONS)).map(
            affiliation -> extractJsonNodeTextValue(affiliation, JSON_PTR_ID)).toList();
    }
    ...
    private static PublicationDate extractPublicationDate(JsonNode resource) {
