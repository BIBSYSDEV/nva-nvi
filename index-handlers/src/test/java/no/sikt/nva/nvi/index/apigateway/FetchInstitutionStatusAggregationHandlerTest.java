...
class FetchInstitutionStatusAggregationHandlerTest {
  ...
  void shouldReturnUnauthorizedWhenUserDoesNotHaveRequiredAccessRight() throws IOException {
    ...
    var request =
        createRequestWithoutAccessRight(institutionId, institutionId, CURRENT_YEAR).build();
  ...
  void shouldReturnOrganizationApprovalStatusAggregation() throws IOException {
    var institutionId = randomUri();
    var request =
        createRequest(institutionId, institutionId, CURRENT_YEAR, MANAGE_NVI_CANDIDATES).build();
